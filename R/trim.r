...
trim <- function(d, b = 4000) {
  ...
  if(class(d)[1] != "grouped_df") stop("Input data must be a tbl_df grouped by individual id")
  
  d.fn <- function(x) distGeo(c(x$lon[1], x$lat[1]), cbind(x$lon, x$lat))
  y <- do(d, mutate(., dist = d.fn(.), home = dist > b)) %>%
    do(., mutate(., newtrip = abs(c(0, diff(!home))))) %>%
